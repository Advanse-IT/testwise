...
function StatCard({ value, suffix, label, index, isOne }) {
  ...
  return (
    <Reveal delay={index * 0.07}>
      <Card className="stat-card border-0 text-center p-5 sm:p-6">
        <div ref={ref} className="text-[30px] sm:text-[36px] font-bold text-brand-teal tracking-tight leading-none mb-2 text-glow">
          {isOne ? `${count} sprint` : `${count}${suffix}`}
        </div>
        <div className="text-[13px] sm:text-[14px] text-white/50 leading-snug">{label}</div>
      </Card>
    </Reveal>
  )
}
...
export default function Home() {
  ...
  return (
    <PageWrapper>
      {/* HERO */}
      <section className="relative min-h-screen flex items-center px-5 sm:px-8 lg:px-12 pt-24 pb-16 overflow-hidden">
        <ParticleCanvas/>
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full">
          <div className="grid lg:grid-cols-[1fr_520px] gap-10 lg:gap-16 items-center">
            ...
            <div>
              <motion.div
                ...
                className="mb-7"
              >
                <Badge variant="default" className="gap-2 py-1.5 px-4 text-[11px]">
              ...
              <motion.h1
                ...
                className="text-hero text-white mb-2 font-bold"
              >
                Autonomous QA,
              </motion.h1>
              ...
              <motion.p
                ...
                className="text-[16px] sm:text-body-xl text-white/55 font-light max-w-lg leading-relaxed mb-3"
              >
              ...
              <motion.p
                ...
                className="text-[13px] sm:text-[14px] text-white/30 mb-9 flex items-center gap-2"
              >
            ...
            {/* QA Visualiser — hidden on mobile, shown lg+ */}
            <motion.div
          ...
          <motion.div
            ...
            className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mt-12 sm:mt-16"
          >
      ...
      <section className="py-20 px-5 sm:px-8 lg:px-12 max-w-7xl mx-auto">
        <Reveal>
          ...
          <h2 className="text-title-xl text-white mb-4">Every stage of QA. End to end.</h2>
        ...
        <Reveal delay={0.1}>
          <div className="beam-container rounded-xl sm:rounded-2xl border border-white/[0.07] bg-card overflow-hidden">
            <div className="beam-line" aria-hidden="true"/>
            <div className="flex overflow-x-auto scrollbar-hide">
              {PIPELINE.map(({ Icon, label, sub }, i) => (
                <div key={i} className="flex items-center flex-shrink-0">
                  <div className="group flex flex-col items-center px-4 sm:px-7 py-7 sm:py-10 transition-colors duration-200 hover:bg-brand-teal/[0.04] cursor-default">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-white/[0.04] border border-white/[0.08] flex items-center justify-center mb-3 sm:mb-4 group-hover:border-brand-teal/30 group-hover:shadow-glow transition-all duration-300">
                      <Icon size={17} className="text-white/35 group-hover:text-brand-teal transition-colors duration-200"/>
                    </div>
                    <span className="text-[11px] sm:text-[13px] font-semibold text-white/70 whitespace-nowrap mb-1">{label}</span>
                    <span className="text-[10px] sm:text-[11px] text-white/25 whitespace-nowrap">{sub}</span>
                  </div>
                  {i < PIPELINE.length - 1 && (
                    <div className="flex-shrink-0 flex items-center gap-0.5 opacity-20">
                      <div className="w-3 sm:w-4 h-px bg-brand-teal"/>
            ...
            <div className="px-5 sm:px-7 py-4 flex items-center justify-between gap-4 bg-white/[0.015]">
              <span className="text-[13px] sm:text-[14px] text-white/30 leading-snug">All stages connected. Gates configurable at any point.</span>
              <Button asChild variant="link" size="sm" className="text-[13px] sm:text-[14px] flex-shrink-0">
                <Link to="/pipeline">Configure <ArrowRight size={12}/></Link>
              </Button>
            </div>
          </div>
        </Reveal>
      ...
      <section className="py-20 px-5 sm:px-8 lg:px-12 max-w-7xl mx-auto">
        <Reveal>
          ...
          <p className="text-[16px] sm:text-body-xl text-white/55 font-light max-w-lg mb-10 sm:mb-14">
            ...
            return (
              <Reveal key={i} delay={i * 0.08}>
                <div className="feature-card p-6 sm:p-7 h-full flex flex-col">
                  ...
                  <h3 className="text-[16px] sm:text-[17px] font-semibold text-white mb-3 tracking-tight leading-snug">{p.title}</h3>
                  <p className="text-[14px] sm:text-[15px] text-white/50 leading-relaxed flex-1 font-light">{p.body}</p>
                </div>
              </Reveal>
            )
          })}
      ...
      <section className="py-20 px-5 sm:px-8 lg:px-12 max-w-7xl mx-auto">
        <Reveal>
          <Card className="overflow-hidden border-brand-teal/15">
            <CardContent className="p-7 sm:p-12 md:p-16 relative">
              <div
              ...
              <div className="relative z-10">
                ...
                <blockquote className="text-[20px] sm:text-title-lg text-white mb-10 max-w-3xl leading-snug font-semibold">
                  "{PROOF.quote}"
                </blockquote>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 sm:gap-8 mb-8 sm:mb-10">
                  {PROOF.metrics.map((m,i) => (
                    <div key={i}>
                      <div className="text-[32px] sm:text-[40px] font-bold text-brand-teal tracking-tight leading-none mb-2 text-glow">{m.value}</div>
                      <div className="text-[13px] sm:text-[14px] text-white/45 font-light leading-snug">{m.label}</div>
                    </div>
                  ))}
                </div>
                <Separator className="bg-white/[0.07] mb-5 sm:mb-6"/>
                <p className="text-[13px] sm:text-[14px] text-white/30 max-w-2xl leading-relaxed">{PROOF.source}</p>
              </div>
            </CardContent>
          </Card>
        </Reveal>
      ...
      {/* CTA */}
      <section className="py-24 sm:py-28 px-5 sm:px-8 text-center relative overflow-hidden">
        ...
        <Reveal className="relative z-10 max-w-2xl mx-auto">
          ...
          <p className="text-[16px] sm:text-body-xl text-white/55 font-light mb-8 sm:mb-10">
